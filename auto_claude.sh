...
MAX_DURATION=""
ENABLE_COMMITS=true
GIT_BRANCH_PREFIX="auto-claude/"
MERGE_STRATEGY="squash"
GITHUB_OWNER=""
GITHUB_REPO=""
WORKTREE_NAME=""
WORKTREE_BASE_DIR="../auto-claude-worktrees"
CLEANUP_WORKTREE=false
LIST_WORKTREES=false
DRY_RUN=false
COMPLETION_SIGNAL="AUTO_CLAUDE_PROJECT_COMPLETE"
...
C_DIM=""
C_RESET=""
C_CLAUDE=""       # Claude's brand color (coral/orange)
C_CLAUDE_LIGHT="" # Lighter version for accents
...
init_colors() {
    if [ "$NO_COLOR" = "true" ] || [ ! -t 2 ]; then
        ...
        # Claude brand colors using 256-color mode for better accuracy
        # Claude's signature coral/orange: approximately #E07A5F / RGB(224,122,95)
        C_CLAUDE='\033[38;5;173m'       # Coral/salmon - closest to Claude's brand
        C_CLAUDE_LIGHT='\033[38;5;216m' # Lighter peach/coral for accents
        
        # Standard colors (adjusted to complement Claude theme)
        C_RED='\033[38;5;203m'          # Soft red
        C_GREEN='\033[38;5;114m'        # Soft green
        C_YELLOW='\033[38;5;221m'       # Warm yellow
        C_BLUE='\033[38;5;110m'         # Soft blue
        C_MAGENTA='\033[38;5;176m'      # Soft magenta
        C_CYAN='\033[38;5;116m'         # Soft cyan
        C_WHITE='\033[0;37m'
        C_BOLD='\033[1m'
        C_DIM='\033[2m'
        C_RESET='\033[0m'
...
start_spinner() {
    ...
    (
        local idx=0
        while true; do
            printf "\r${C_CLAUDE}${SPINNER_FRAMES[$idx]}${C_RESET} %s" "$message" >&2
...
show_config_summary() {
    ...
    echo -e "  ${C_CLAUDE}📝 Prompt:${C_RESET} $prompt_display" >&2
    ...
    echo -e "  ${C_CLAUDE}🎯 Limits:${C_RESET} $limits" >&2
    
    # Repository
    if [ "$ENABLE_COMMITS" = "true" ]; then
        echo -e "  ${C_CLAUDE}📦 Repository:${C_RESET} $GITHUB_OWNER/$GITHUB_REPO" >&2
        echo -e "  ${C_CLAUDE}🔀 Merge Strategy:${C_RESET} $MERGE_STRATEGY" >&2
    else
        echo -e "  ${C_CLAUDE}📦 Repository:${C_RESET} ${C_DIM}commits disabled${C_RESET}" >&2
    fi
    
    # Worktree
    [ -n "$WORKTREE_NAME" ] && echo -e "  ${C_CLAUDE}🌿 Worktree:${C_RESET} $WORKTREE_NAME" >&2
    
    # Completion settings
    echo -e "  ${C_CLAUDE}🎉 Auto-stop:${C_RESET} After $COMPLETION_THRESHOLD completion signals" >&2
...
show_realtime_stats() {
    ...
    [ -n "$total_cost" ] && [ "$(awk "BEGIN {print ($total_cost > 0)}")" = "1" ] && printf "${C_DIM}│${C_RESET}  Cost: ${C_CLAUDE}\$%.3f${C_RESET}                                 ${C_DIM}│${C_RESET}\n" "$total_cost" >&2
...
show_help() {
    cat << EOF
Auto-Claude - Run Claude Code iteratively with automatic PR management

USAGE:
    auto-claude -p "prompt" (-m max-runs | --max-cost max-cost | --max-duration duration) [--owner owner] [--repo repo] [options]
    auto-claude update
...
OPTIONAL FLAGS:
    ...
    --git-branch-prefix <prefix>  Branch prefix for iterations (default: "auto-claude/")
    ...
    --worktree-base-dir <path>    Base directory for worktrees (default: "../auto-claude-worktrees")
    --cleanup-worktree            Remove worktree after completion
    --list-worktrees              List all active git worktrees and exit
    --dry-run                     Simulate execution without making changes
    --completion-signal <phrase>  Phrase that agents output when project is complete (default: "AUTO_CLAUDE_PROJECT_COMPLETE")
...
EXAMPLES:
    ...
    auto-claude -p "Fix all linter errors" -m 5 --owner myuser --repo myproject

    # Run with cost limit
    auto-claude -p "Add tests" --max-cost 10.00 --owner myuser --repo myproject

    # Run for a maximum duration (time-boxed)
    auto-claude -p "Add documentation" --max-duration 2h --owner myuser --repo myproject
    
    # Run for 30 minutes
    auto-claude -p "Refactor module" --max-duration 30m --owner myuser --repo myproject

    # Run without commits (testing mode)
    auto-claude -p "Refactor code" -m 3 --disable-commits

    # Use custom branch prefix and merge strategy
    auto-claude -p "Feature work" -m 10 --owner myuser --repo myproject \\
        --git-branch-prefix "ai/" --merge-strategy merge

    # Combine duration and cost limits (whichever comes first)
    auto-claude -p "Add tests" --max-duration 1h30m --max-cost 5.00 --owner myuser --repo myproject

    # Run in a worktree for parallel execution
    auto-claude -p "Add unit tests" -m 5 --owner myuser --repo myproject --worktree instance-1

    # Run multiple instances in parallel (in different terminals)
    auto-claude -p "Task A" -m 5 --owner myuser --repo myproject --worktree task-a
    auto-claude -p "Task B" -m 5 --owner myuser --repo myproject --worktree task-b

    # List all active worktrees
    auto-claude --list-worktrees

    # Clean up worktree after completion
    auto-claude -p "Quick fix" -m 1 --owner myuser --repo myproject \\
        --worktree temp --cleanup-worktree

    # Use completion signal to stop early when project is done
    auto-claude -p "Add unit tests to all files" -m 50 --owner myuser --repo myproject \\
...
NOTE:
    auto-claude automatically checks for updates at startup. You can press 'N' to skip the update.

For more information, visit: https://github.com/iKislay/auto-claude
EOF
}

show_version() {
    echo "auto-claude version $VERSION"
}
...
get_latest_version() {
    ...
    latest_version=$(gh release view --repo iKislay/auto-claude --json tagName --jq '.tagName' 2>/dev/null)
...
download_and_install_update() {
    ...
    local download_url="https://raw.githubusercontent.com/iKislay/auto-claude/${latest_version}/auto_claude.sh"
    local checksum_url="https://raw.githubusercontent.com/iKislay/auto-claude/${latest_version}/auto_claude.sh.sha256"
...
check_for_updates() {
    ...
    if [ $comparison -eq 1 ]; then
        ...
        echo "" >&2
        echo "🆕 A new version of auto-claude is available: $latest_version (current: $VERSION)" >&2
        ...
        if [[ "$response" =~ ^[Yy]$ ]]; then
            ...
            echo "⏭️  Skipping update. You can update later with: auto-claude update" >&2
        fi
    fi
...
handle_iteration_success() {
    ...
    if [ "$ENABLE_COMMITS" = "true" ]; then
        ...
        if ! auto_claude_commit "$iteration_display" "$branch_name" "$main_branch"; then
            error_count=$((error_count + 1))
            extra_iterations=$((extra_iterations + 1))
    ...
    # Show real-time stats after each successful iteration
    show_realtime_stats
...
execute_single_iteration() {
    ...
    # Show progress bar if max runs is set
...
"
    ...
    show_step 2 5 "Running Claude Code..." "running"
    
    # Start spinner for visual feedback during long-running Claude execution
    start_spinner "Claude Code is working... (this may take a few minutes)"
    ...
    if [ $claude_exit_code -ne 0 ]; then
        show_step 2 5 "Claude Code failed (exit code: $claude_exit_code)" "error"
    ...
    show_step 2 5 "Claude Code completed" "success"
...
main_loop() {
    # Always initialize start time for elapsed time tracking
    start_time=$(date +%s)
...
show_completion_summary() {
    ...
    echo -e "  ${C_CLAUDE}⏱${C_RESET}  Total time: $(format_duration $elapsed_time)" >&2
    ...
    # Next steps hint (only if project not complete)
    if [ $completion_signal_count -lt $COMPLETION_THRESHOLD ] && [ "$QUIET_MODE" != "true" ]; then
        echo -e "  ${C_BOLD}💡 Next Steps${C_RESET}" >&2
        echo -e "  ─────────────────────────────────────────" >&2
        echo -e "  • Check ${C_CLAUDE}$NOTES_FILE${C_RESET} for iteration notes" >&2
        echo -e "  • Re-run with ${C_DIM}auto-claude -p \"...\" -m 5${C_RESET} to continue" >&2
        echo "" >&2
    fi
}
...
main() {
    ...
    # Initialize colors after parsing (to respect --no-color flag)
    init_colors
