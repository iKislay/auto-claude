...
setup() {
    ...
    SCRIPT_PATH="$BATS_TEST_DIRNAME/../auto_claude.sh"
    export TESTING="true"
}
...
@test "show_help displays help message" {
    ...
    export -f show_help
    run show_help
    assert_output --partial "Auto-Claude - Run Claude Code iteratively"
...
@test "show_version displays version" {
    ...
    run show_version
    assert_output --partial "auto-claude version"
}
...
@test "parse_arguments sets default completion values" {
    ...
    assert_equal "$COMPLETION_SIGNAL" "AUTO_CLAUDE_PROJECT_COMPLETE"
...
@test "get_script_path returns script path" {
    ...
    function readlink() {
        if [ "$1" = "-f" ]; then
            echo "/usr/local/bin/auto-claude"
            return 0
        fi
        return 1
    }
    ...
    assert_success
    assert_output "/usr/local/bin/auto-claude"
}
    ...
    function curl() {
        ...
        if [ -n "$output_file" ]; then
            ...
            if [[ "$output_file" == *".sha256"* ]] || [[ "${@}" == *".sha256"* ]]; then
                ...
                echo "dummychecksum123456789  auto_claude.sh" > "$output_file"
            else
...
@test "check_for_updates with skip_prompt does not prompt" {
    ...
    assert_success
    assert_output --partial "A new version of auto-claude is available"
...
@test "auto_claude_commit dry run shows PR merged message with placeholder" {
    source "$SCRIPT_PATH"
    ...
    run auto_claude_commit "(1/1)" "test-branch" "main"
